...
TOKEN_SEP = ":"
microJy = float(1.0e-23 * 1.0e-06)
nanoJy = float(1.0e-23 * 1.0e-09)
...
class ObservedSpectrum(object):
        ...
        for key, value in six.iteritems(self.description):
            ...
            if isinstance(value, dict):
                for in_key, in_value in six.iteritems(value):
                    ...
                    if in_token in line:
                        self.description[key][in_key] = line.split(in_token)[1].split(
                            " "
                        )[0]
    ...
    def load(self, file_name):
        ...
        if self.description is None:
            msg = (
                "The `ObservedSpectrum.description` must be set through "
        ...
        try:
            ...
            msg = (
                "`" + self.description["wl"]["colName"] + "` not found "
        ...
        try:
            ...
            msg = (
                "`" + self.description["flux"]["colName"] + "` not found "
                "in the current spectrum!"
            )
            raise AttributeError(msg)
        ...
        if self.description["fluxerr"]["colName"] is not None:
            try:
                self.data["fluxerr"] = np.array(
                    data[self.description["fluxerr"]["colName"]]
                )
            except:
                msg = (
                    "`" + self.description["fluxerr"]["colName"] + "` not found "
                    "in the current spectrum!"
                )
                raise AttributeError(msg)
        ...
        if self.description["redshift"]["keyword"] is not None:
            try:
                self.data["redshift"] = float(
                    hdu[1].header[self.description["redshift"]["keyword"]]
                )
            except:
                pass
        ...
        if self.description["mask"]["colName"] is not None:
            self.data["mask"] = np.array(
                data[self.description["mask"]["colName"]], dtype=bool
            )
...
class Spectrum(object):
    ...
    def __init__(self, param_file=None, config=None, **kwargs):

        self.inset_fontsize = (
            BeagleDirectories.inset_fontsize_fraction * BeagleDirectories.fontsize
        )
        ...
        if kwargs.get("line_labels_json") is None:
            self.line_labels = json.load(
                resource_stream(__name__, "files/emission_lines.json"),
        ...
        self.show_calibration_correction = kwargs.get(
            "show_calibration_correction", False
        )
        ...
        if self.show_calibration_correction:
            ...
            self.calibration_correction.configure(
                config,
                os.path.join(
                    BeagleDirectories.results_dir,
                    kwargs.get("json_calibration_correction"),
                ),
            )
    ...
    def plot_marginal(
        ...
        if self.wl_rest:
            plot_name = (
                str(ID) + "_BEAGLE_marginal_SED_spec_rest_wl" + plot_suffix + ".pdf"
            )
        else:
        ...
        if (
            ...
            and self.calibration_correction.has_correction
        ):
            ...
            for i in range(model_fluxes.shape[0]):
                ...
                for d in range(self.calibration_correction.degree + 1):
                    ...
                    if self.calibration_correction.coeff_params[label]["fitted"]:
                        ...
                        tmp_coeff.append(
                            self.calibration_correction.coeff_params[label]["value"]
                        )

                calibration_correction_arr[i, :] = (
                    self.calibration_correction.return_correction(
                        (model_wl - w0) / 1e4, tmp_coeff
                    )
                )
        ...
        if self.wl_rest:
            if multiple_redshifts:
                logging.warning(
                    "Selecting the maximum-a-posteriori redshift to plot in the rest-frame"
                )
            z1 = 1.0 + redshift
        ...
        pos_err = np.where(data_flux_err > 0.0)[0]
        ...
        if "marginal sed mask" in hdulist:
            model_mask = np.array(
                hdulist["marginal sed mask"].data["mask"][0], dtype=bool
            )
        ...
        if n_ranges == 1:
            ...
            wl_ranges = [
                (self.wl_range[2 * i], self.wl_range[2 * i + 1])
            ...
            if self.show_residual:
                ...
                if (
                    ...
                    t = (
                        calibration_axs[0].get_figure().get_dpi()
                        / (t[1, 1] - t[0, 1])
                        / 72
                    )
                    d = 0.5 * (rcParams["xtick.major.size"] * t)
                    ...
                    for i, (ax_l, ax_r) in enumerate(pairwise(calibration_axs)):
                        kwargs = dict(
                            transform=ax_l.transAxes, color="k", clip_on=False
                        )
                        ax_l.spines["right"].set_visible(False)
                        if not ax_l.spines["left"].get_visible():
                            ax_l.yaxis.set_ticks_position("none")
                        ...
                        kwargs = dict(
                            transform=ax_r.transAxes, color="k", clip_on=False
                        )
                        ax_r.spines["left"].set_visible(False)
                        ax_r.yaxis.tick_right()
                        ax_r.tick_params(labelright="off")
        ...
        if self.show_residual:
            if self.residual_units == "relative":
                ...
                ylabel = (
                    "$\\frac{F_{\\uplambda}-F_{\\uplambda}^\\textnormal{mod}}{\\sigma}$"
                )
        ...
        if self.print_ID:
            ...
            plt.suptitle(
                "ID: " + str(ID).split("_")[0].strip(), fontsize=self.inset_fontsize
            )
        ...
        for ax in axs:
            ...
            for slic, col in zip(slices, colors):
                for s in slic:
                    ...
                    if self.draw_steps:
                        FillBetweenStep.fill_between_steps(
                            ax,
                            data_wl[s],
            ...
            if "full sed wl" in hdulist:
                ...
                if self.plot_full_SED:
                    ...
                    for i in rand_indices:
                        ...
                        if self.wl_rest:
                            ...
                            _z1 = 1.0 + hdulist["galaxy properties"].data["redshift"][i]
                            wl_obs = (
                                hdulist["full sed wl"].data["wl"][0, :]
                                * _z1
                                / wl_factor
                            )
                        ...
                        ax.plot(
                            wl_obs, flux_obs, color="grey", ls="-", lw=0.3, alpha=0.3
                        )
                ...
                if self.plot_MAP_SED:
                    ...
                    if self.wl_rest:
                        ...
                        _z1 = 1.0 + hdulist["galaxy properties"].data["redshift"][i]
                        wl_obs = (
                            hdulist["full sed wl"].data["wl"][0, :] * _z1 / wl_factor
                        )
                        flux_obs = hdulist["full sed"].data[i, :] / _z1

                    ax.plot(wl_obs, flux_obs, color="black", ls="-", lw=0.6, alpha=0.6)
        ...
        if self.show_residual:
            ...
            if self.residual_units == "relative":
                ...
                residual_err = (
                    1.0 / data_flux_ - (data_flux_ - median_flux) / data_flux_**2
                ) * data_flux_err_
            elif self.residual_units == "absolute":
        ...
        for ax in axs:
            ...
            if self.plot_line_labels:
                ...
                if observation.data["redshift"] is not None:
                    z1 = 1.0 + observation.data["redshift"]
                else:
                    z1 = (
                        1.0
                        + hdulist["galaxy properties"].data["redshift"][
                            np.argmax(probability)
                        ]
                    )
            ...
            if print_text:
                ...
                try:
                    ax.text(
                        x,
                        y,
                        "$\\log(Z)=" + "{:.2f}".format(self.logEvidence) + "$",
                        fontsize=self.inset_fontsize,
                    )
                ...
                try:
                    aver_chi_square = self.PPC.data["aver_chi_square"][
                        self.PPC.data["ID"] == ID
                    ]
                    y = y1 - (y1 - y0) * 0.15
                    ax.text(
                        ...
                        y,
                        "$\\langle\chi^2\\rangle="
                        + "{:.2f}".format(aver_chi_square)
                        + "$",
                        fontsize=self.inset_fontsize,
                    )
                except AttributeError:
                    print(
                        "`PosteriorPredictiveChecks` not computed/loaded, hence "
                        "<chi^2> for the object `" + str(ID) + "` is not available"
                    )

                try:
                    aver_red_chi_square = self.PPC.data["aver_red_chi_square"][
                        self.PPC.data["ID"] == ID
                    ]
